  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col [background:var(--cl-card)] [color:var(--cl-ink)]">
      ...
      <div className="flex-1 overflow-y-auto p-4">
          ...
          {images.map((img, i) => (
            <div key={i} className="relative aspect-square">
              ...
              <button
                ...
                className="absolute -right-1.5 -top-1.5 flex h-6 w-6 items-center justify-center rounded-full [background:var(--cl-ink)] text-white shadow"
              >
                <XIcon className="h-3.5 w-3.5" />
              </button>
            </div>
          ))}
          <label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed [border-color:var(--cl-line)] [color:var(--cl-ink-3)]">
        ...
        {images.length === 0 && (
          <p className="mt-4 text-center text-sm [color:var(--cl-ink-2)]">No photos yet. Tap the tile above to add one.</p>
        )}
      </div>
    </div>
  )
